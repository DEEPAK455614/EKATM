...
export async function POST(req:NextRequest){
  try{
    ...
    const {data:existingProject}=await db.from('survey_projects').select('created_by').eq('id',draft.projectId).maybeSingle();
    const project={id:draft.projectId,state_name:draft.state.trim(),survey_period_start:draft.surveyStart||null,survey_period_end:draft.surveyEnd||null,status:draft.status||'draft',revision:draft.revision||1,created_by:existingProject?.created_by||user.id,updated_at:new Date().toISOString()};
    ...
    // Replace state-level master-survey data, while preserving append-safe daily field reports.
    for(const table of ['state_profiles','district_features','survey_decisions_followups','committees','final_route_days']){
      const {error}=await db.from(table).delete().eq('survey_project_id',draft.projectId);
      if(error) throw error;
    }
    const {error:masterLogDelete}=await db.from('daily_logs').delete().eq('survey_project_id',draft.projectId).is('client_day_key',null);
    if(masterLogDelete) throw masterLogDelete;
    for(const table of ['survey_organization_visits','gathering_venues','night_halts','rath_yatra_plans','events_festivals','strategic_actions']){
      const {error}=await db.from(table).delete().eq('survey_project_id',draft.projectId).is('daily_log_id',null);
      if(error) throw error;
    }
    ...
    const {error:teamDelete}=await db.from('survey_team_members').delete().eq('survey_project_id',draft.projectId); if(teamDelete) throw teamDelete;
