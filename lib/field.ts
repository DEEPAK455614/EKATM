import type { ContactObservation, Festival, NightHalt, OrgVisit, RathPlan, StrategicAction, Venue } from './survey';
...
export function newFieldDay(projectId:string,state:string,dayNo=1):FieldDayDraft{
  const now=new Date();
  return {id:id(),projectId,state,date:now.toISOString().slice(0,10),dayNo,teamLabel:'',journeyFrom:'',journeyTo:'',distance:'',persons:[],organizations:[],otherCoveredPlaces:'',completedTasks:'',keyFindings:'',organizationVisits:[],rathPlans:[],festivals:[],strategicActions:[],status:'draft',startedAt:now.toISOString()};
}

export function dayCompleteness(d:FieldDayDraft){
  const checks=[Boolean(d.date),Boolean(d.journeyFrom||d.journeyTo),Boolean(d.completedTasks),Boolean(d.keyFindings),d.persons.length>0||d.organizations.length>0,d.organizationVisits.length>0||d.rathPlans.length>0||d.festivals.length>0||d.strategicActions.length>0];
