'use client';
import {statusLabel} from '@/lib/simple-survey';
...
export default function SurveyHistory({formId}:{formId:string}){
 ...
 return <details className={s.sectionCard} style={{margin:'16px 0'}}><summary>Saved version history ({rows.length}{rows.length===50?'+':''})</summary><p>Latest 50 saved versions. Each records the survey exactly as it was saved at that time.</p>{message&&<p role="status">{message}</p>}{rows.map(r=><div className={s.readItem} key={r.id}><b>Version {r.revision} · {statusLabel(r.status)}</b><p>{new Date(r.created_at).toLocaleString()} · {r.review_notes}</p><button className={s.secondary} onClick={()=>download(r.id)}>Download this version</button></div>)}</details>;
}
